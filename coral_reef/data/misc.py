...
def get_mask_names_for_color(mask_folder_path, colour, fetch_type="all", file_extension="png"):
    """
    Returns image_paths for a given fetch_type
    :param mask_folder_path: path to the folder containing the images
    ...
    # get image files
    image_files = glob.glob(os.path.join(mask_folder_path, "*." + file_extension))
    res_paths = []
...
def calc_class_shares(mask_folder_path, colour_mapping, file_extension="png"):
    """
    Calculates the class distribution for given mask files
    :param mask_folder_path: path to the folder containing the masks
...
def calculate_class_stats(mask_folder_path, colour_mapping):
    """
    ...
    shares = calc_class_shares(mask_folder_path, colour_mapping)
    ...
    for c in classes:
        ...
        stats[c]["max_file"] = get_mask_names_for_color(mask_folder_path, colour_mapping[c], fetch_type="max")[0]
        stats[c]["files"] = get_mask_names_for_color(mask_folder_path, colour_mapping[c], fetch_type="all")
        pbar.update(1)
...
if __name__ == "__main__":
    ...
    # out_folder_path = "/home/aljo/filament/coral_reef/data/masks_small"
    # shrink_images(in_folder_path, out_folder_path, scale_factor=0.25)
    ...
    stats = calculate_class_stats(mask_folder_path, colour_mapping)

    with open("/home/aljo/filament/coral_reef/data/class_stats.json", "w") as fp:
